...
public sealed class TicketSlaTests
{
    ...
    /// <summary>
    /// Verifies that a later policy does not recalculate an existing deadline.
    /// </summary>
    [Fact]
    public void Existing_ticket_should_keep_original_sla_snapshot()
    {
        DateTime createdAtUtc = new(
            2026,
            9,
        ...
        Assert.Equal(originalPolicy.Id, ticket.SlaPolicyId);
        Assert.Equal(createdAtUtc.AddDays(2), ticket.SlaDeadlineUtc);
    }
